...
bool PT63DriveController::Init()
{
    ...
    OH_LOG_INFO(LOG_APP, "PT63DriveController::Init");
    this->IDeviceStatus->RegisterResetNotice(this);
    OH_LOG_INFO(LOG_APP, "PT63DriveController::Reset");
    this->IDeviceStatus->Reset();
    return true;
}
...
//            OH_LOG_ERROR(LOG_APP, "DSCP_EVENT_OAI_SIGNAL_AD_NOTICE [%{public}d][%{public}d]", m_MeasureAD[0], m_MeasureAD[1]);
    ...
    //        }
            ...
            break;
        case DSCP_EVENT_DSI_RESET:
            OH_LOG_ERROR(LOG_APP, "下位机复位");
            break;
        default:
            break;
    }
...
//    if (this->GetConnectStatus())
    {
        ...
        {
            OH_LOG_ERROR(LOG_APP, "PT63DriveController::ResetHandler() => %{public}s", e.What().c_str());
        }
