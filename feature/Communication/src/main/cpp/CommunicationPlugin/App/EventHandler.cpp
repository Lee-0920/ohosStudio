...
//    luaEngine->GetLuaValue(state, "config.system.debugMode", m_isPrint);
    ...
    if (pthread_create(&tid, NULL, &EventHandler::ThreadEntry, this) != 0) {
        ...
        OH_LOG_INFO(LOG_APP, "EventHandler thread created successfully");
...
//                              event->ToString().c_str());
                OH_LOG_INFO(LOG_APP, "m_remainEvents-- = %{public}d, %{public}s",
                              m_remainEventList.size(),
                              event->ToString().c_str());
                break;
            }
...
{
    DscpEventPtr event = nullptr;
    OH_LOG_INFO(LOG_APP, "EventHandler Expect code = %{public}x", (int)code);
    ...
    OH_LOG_INFO(LOG_APP, "EventHandler waiting for code = %{public}x, timeout = %{public}d ms",
                (int)code, timeout);

    pthread_cond_t waitCondition;
    pthread_cond_init(&waitCondition, nullptr);
    ...
    // ✅ 关键修复：不再检查栈上 expectInfo.event
    // timedwait 本身就会在被 signal 唤醒后立即返回（返回值为0）
    // 超时则返回 ETIMEDOUT（非0）
    int waitResult = pthread_cond_timedwait(&waitCondition, &m_expectMutex, &absTimeout);

    // ✅ 从链表中获取通知方写入的 event（而非栈上副本）
    for (auto it = m_expectList.begin(); it != m_expectList.end(); ++it) {
        if (it->addr.a1 == addr.a1 && it->addr.a2 == addr.a2 &&
            it->addr.a3 == addr.a3 && it->addr.a4 == addr.a4 &&
            it->code == code) {
            event = it->event;  // ← 这里取的才是通知方设置的那个值
    ...
    if (event == nullptr) {
        event = GetEvent(addr, code);
        if (event == nullptr) {
            OH_LOG_ERROR(LOG_APP, "EventHandler Expect timeout! code = %{public}x, waitResult = %{public}d",
                         (int)code, waitResult);
...
//    logger->debug("EventHandler thread ID = %d", QThread::currentThreadId());
    ...
    OH_LOG_INFO(LOG_APP, "LaiSerial Thread ID = %{public}d", (int)tid);
    while (true)
    {
...
//                    logger->debug(event->ToString());
                    OH_LOG_INFO(LOG_APP, "%{public}s", event->ToString().c_str());
                }
